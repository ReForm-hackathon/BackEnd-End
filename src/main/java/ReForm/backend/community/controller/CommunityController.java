...
public class CommunityController {
    ...
    /**
     * 커뮤니티 댓글 조회
     * - 경로: GET /board/{board_id}/see-comment
     * - 헤더: Authorization: Bearer {access_token}
     */
    @GetMapping("/board/{boardId}/see-comment")
    public ResponseEntity<Map<String, Object>> getCommunityComments(@PathVariable Integer boardId) {
        try {
            String userId = getCurrentUserId();
            if (userId == null) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
            ...
            var comments = communityCommentRepository.findByCommunity_CommunityIdOrderByCreatedAtDesc(boardId);
            ...
            log.error("[/board/{}/see-comment] 조회 실패", boardId, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(Map.of("error", "서버 에러가 발생했습니다."));
        }
    }
